...
Nonetheless after some tinkering I finally managed to get a nice JSON message over the MQTT Broker: 
\begin{lstlisting}
{ 
    "id": "SENSOR-XSZ", 
...
Protobuf would be a great alternative, which I will try to implement as well. Protobuff does take some time to set up but would simplify the data transfer greatly since bytes could be directly sent and would not need to be concatenated to a JSON.
